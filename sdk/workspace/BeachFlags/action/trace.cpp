...
// コンストラクタ（オーバーロード）
Trace::Trace(LeftOrRight lineEdge, double Kp, double Ki, double Kd, int speedMin, int speedMax)
...
Trace::Trace(LeftOrRight lineEdge, double Kp, double Ki, double Kd, int steeringMin, int steeringMax, int speedMin, int speedMax)
    : Run(speedMin, speedMax),
      mLineEdge(lineEdge), mKp(Kp), mKi(Ki), mKd(Kd), mError(),
...
Trace::Trace(LeftOrRight lineEdge, double Kp, double Ki, double Kd, int steeringMin, int steeringMax, int speedMin, int speedMax, int steeringOffset)
    : Run(speedMin, speedMax),
      mLineEdge(lineEdge), mKp(Kp), mKi(Ki), mKd(Kd), mError(),
...
// 旋回量を決定（PID制御：条件付き積分）
void Trace::determineSteering()
{
    double u = 0.0, dt = 0.01;
    double center = (gWhite + gBlack) / 2.0;
    ...
    mError[0] = hosei(brightness - center);
